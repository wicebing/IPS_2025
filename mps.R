...
indoorPosition <- read.csv("C:/Users/USER/Downloads/IPS/loading_event.csv")
...
t.test(mps~event_c, alternative='two.sided', conf.level=.95, 
       var.equal=FALSE, data=data_all)
t.test(mps~event_f, alternative='two.sided', conf.level=.95, 
       var.equal=FALSE, data=data_all)

GLM.2 <- glm(event_f ~ mps + hour + weekday, 
             family=binomial(logit), data=indoorPosition)
summary(GLM.2)
exp(coef(GLM.2))  # Exponentiated coefficients ("odds ratios")

GLM.3 <- glm(event_c ~ mps + hour + weekday, 
             family=binomial(logit), data=indoorPosition)
summary(GLM.3)
exp(coef(GLM.3))  # Exponentiated coefficients ("odds ratios") 看分布面積對事件的影響

GLM.4 <- glm(mps ~ event_c + hour + weekday, 
             family=binomial(logit), data=indoorPosition)
summary(GLM.4)
