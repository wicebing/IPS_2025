...
def plot_trajectory(dfs, pic_name='evtTimePoint',grid=False, evt=0):
    ...
    if evt == 0:
        ...
        x = [5,20.5,20.5,12.5,12.5,4,4,10]
        y = [6,6,23.5,23.5,8,8,14.5,14.5]
        x = scale*np.array(x)
        y = scale*np.array(y)
        ax.plot(x, y, color='red', alpha = 0.5, linestyle = '-',linewidth=10)
...
def plot_all_trajectories(events, drawPds, grid=False):
    """Plots all events on a single map."""
    dfs = {}
    for i, evt in events.iterrows():
        print(f' == work on {i} event == ')
        endtime = evt['endTime']
        ...
        for beacon in select_beacons:
            df = drawPds[beacon].loc[(drawPds[beacon]['positionTime'] >= startTime) & (drawPds[beacon]['positionTime'] <= endtime)]
            if beacon not in dfs:
                dfs[beacon] = df
            else:
                dfs[beacon] = pd.concat([dfs[beacon], df])
    
    plot_trajectory(dfs, pic_name='all_events', grid=grid, evt=-1)
...
# Plot all events on a single map
plot_all_trajectories(events[:2], txyzPds, grid=True)
